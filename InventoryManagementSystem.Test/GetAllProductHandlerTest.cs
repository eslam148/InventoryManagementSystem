...
namespace InventoryManagementSystem.Test
{
    ...
    {
        [Fact]
        public async Task Handle_ValidQuery_ShouldGetAllProductAndBaseReponseGenericOfBoolHaveData()
        {
            ...
            var handler = new GetAllProductsHandler(uow);

            var query = new GetAllProductsQuery();

            // Act
            BaseReponseGeneric<List<GetProductDto>> result = await handler.Handle(query, CancellationToken.None);

            // Assert
            Assert.IsType<BaseReponseGeneric<List<GetProductDto>>>(result);
